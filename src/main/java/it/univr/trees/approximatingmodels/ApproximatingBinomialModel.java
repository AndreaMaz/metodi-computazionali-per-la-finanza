...
public abstract class ApproximatingBinomialModel {
	...
	/*
	 * This is of fundamantal importance in our implementation: we give it the value of the possible up and down
	 * movements according to the specific model we consider (that is, these are computed in derived classes of
	 * this abstract one), and then we delegate to it the implementation of the methods where we want to get values, the probabilities
	 * of the states etc..
	 */
	...
	/*
	 * In this method we generate the binomialModel. Note that, once we know the up and ddown factors, the
	 ...
	 * @param DoubleUnaryOperator transformFunction, the function
	 * @return an array of doubles representing a function all the possible values of the binomial model at timeIndex.
	 * 		   The value in position i is B(0)*transformFunction(u^(Math.round(time/timeStep)-i)*d^i)
	 */
