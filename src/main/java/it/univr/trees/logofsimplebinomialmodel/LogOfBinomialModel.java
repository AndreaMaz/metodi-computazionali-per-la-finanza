...
/**
 * This class is used in order to construct the logarithm a binomial model. The binomial model is a discrete model
 ...
 * In this simple case, we assume r=0.
 * This is done under the risk neutral measure for log(S): it can be proved (see the exercise sheet solution)
 * that it has to hold
 * q=(log(d)-log(u))/log(d).
 ...
 */
public class LogOfBinomialModel {
	...
	private double[][] valuesWithoutLog;
	 ...
	 */
	public LogOfBinomialModel(double upFactor, double downFactor, double initialValue, int numberOfTimes) {
		...
		riskNeutralProbabilityUp =  Math.log(downFactor)/(Math.log(downFactor)-Math.log(upFactor));
	...
	private void generateValues() {
		valuesWithoutLog = new double[numberOfTimes][numberOfTimes];
		valuesWithoutLog[0][0] = Math.exp(initialValue);//we have to take the exponential of the first value log(S_0)
		...
		for (int numberOfMovements = 1; numberOfMovements < numberOfTimes; numberOfMovements++) {
			for (int numberOfUps = 0; numberOfUps <= numberOfMovements; numberOfUps++) {
				 ...
				 */
				valuesWithoutLog[numberOfMovements][numberOfDowns] = valuesWithoutLog[0][0] * Math.pow(upFactor, numberOfUps)*
	...
	//In the next method we just generate S, then we apply the logarithm in the public methods (this is just a choice)
	...
	/**
	 * It returns all the possible values of the logarithm of the binomial model at the given time index.
	 * The element in position i is the one where the underlying has gone down i times.
	 * @param timeIndex, the given time index
	 * @return an array of doubles representing all the possible values of the logarithm of the binomial model
	 * 			at timeIndex. The value in position i is log(B(0)*u^(timeIndex-i)*d^i)
	 */
	public double[] getValuesAtGivenTimeIndex(int timeIndex) {
		 ...
		 */
		double[] valuesAtGivenTimeIndexWothoutLog = Arrays.copyOfRange(valuesWithoutLog[timeIndex], 0, timeIndex+1);
		// right before returning, we apply the logarithm function
		return UsefulMethodsForArrays.applyFunctionToArray(valuesAtGivenTimeIndexWothoutLog, x -> Math.log(x));
	}

	/**
	 * It returns an array whose elements are a function of all the possible values of the logarithm of the binomial model at
	 ...
	 * @return an array of doubles representing a function all the possible values of the logarithm of the binomial model at timeIndex.
	 * 		   The value in position i is log(transformFunction(B(0)(u^(timeIndex-i)*d^i)))
	 */
	...
	/**
	 * It returns the probabilities of all the possible values of the logarithm of the binomial model at the given time index.
	 ...
	 * 		   model at timeIndex. The value in position i is the probability of log(B(0)*u^(timeIndex-i)*d^i)
	 */
	...
	public double[] getConditionalExpectation(double[] binomialValues,int timeIndex) {
		...
		for (int i = 0; i <= timeIndex; i++) {
			 ...
			 */
			conditionalExpectation[i] = (binomialValues[i]*riskNeutralProbabilityUp + binomialValues[i + 1]*riskNeutralProbabilityDown);
		}
